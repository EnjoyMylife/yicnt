    ...
    <header class="yc-main-header">
        ...
        <div class="yc-header-menu-left">
            <template v-for="item in navbarList" >
                <div class = "topbar-item fl" :checked="item.id == navbarActive ? true : false" :key="item.id" @click.stop="changeNavbar(item)">
                    <span class = "topbar-item-btn"><i :class="item.attributes.icon"></i> {{item.text}}</span>
                </div>
            </template>
        </div>
    ...
    methods: {
        ...
        getMenuData() {
            ...
            // narbarApi.getMenuData().then((res) => {
                ...
                _this.navbarList = [
                    { 'id': '1', 'attributes': { 'icon': 'fa fa-battery-0' }, 'text': '联合办公', 'children': [
                        { 'id': '7', 'text': '这是文字你懂的1', 'hasChild': true, 'attributes': { 'icon': 'fa fa-battery-0' }, 'children': [{ 'id': '11', 'text': '这是文字1', 'attributes': { 'url': '/page1' } }, { 'id': '12', 'text': '这是文字2', 'attributes': { 'url': '/page2' } }] },
                        { 'id': '8', 'text': '这是文字你懂的2', 'hasChild': true, 'attributes': { 'icon': 'fa fa-battery-0' }, 'children': [{ 'id': '13', 'text': '这是文字11', 'attributes': { 'url': '/page11' } }] },
                        { 'id': '9', 'text': '这是文字你懂的3', 'hasChild': false, 'attributes': { 'icon': 'fa fa-battery-0', 'url': '/page3' } }
                    ]},
                    { 'id': '2', 'attributes': { 'icon': 'fa fa-battery-1' }, 'text': '产业园', 'children': [
                        { 'id': '10', 'text': '这是文字你懂的4', 'hasChild': true, 'attributes': { 'icon': 'fa fa-battery-0' }, 'children': [{ 'id': '14', 'text': '这是文字11', 'attributes': { 'url': '/page11' } }] }
                    ]},
...
<style lang="scss" scoped>
    .yc-main-header {
        ...
        height: 50px;
        box-shadow: 12px 3px 5px rgba(1, 1, 1, .2);
        position: fixed;
        top: 0;
        z-index: 666;
        width: 100%;
