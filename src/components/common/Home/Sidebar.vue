<template>
    <div class="yc-sidebar" v-loading="loading">
        <div :class="'yc-sidebar-toggle ' + (sidebar.opend ? 'close' : '')" @click.stop="toggleSideBar">
        ...
        <el-menu
            ...
            <template v-for="item in sidebar.data" :index="item.id">
                <el-menu-item v-if="!item.hasChild" :key="item.text" :index="item.id" @click="addTab(item)">
                    <template slot="title">
                ...
                <el-submenu v-else :key="item.text" :index="item.id">
                    ...
                    <template>
                        <el-menu-item v-for="child in item.children" :key="child.text" :index="child.id" @click="addTab(child)" :title="child.text">{{ child.text }}</el-menu-item>
                    </template>
                </el-submenu>
            </template>
        </el-menu>
