<template>
    <div class="yc-tab">
        <v-contextmenu ref="tabmenu">
            <v-contextmenu-item v-for="item of list" :key="item.id" @click="handleClick(item.id)">{{item.label}}</v-contextmenu-item>
        </v-contextmenu>
        <div class="yc-tab-links">
            <ul>
                <transition-group name="tabs">
                    <li v-contextmenu:tabmenu
                        ...
                        <div class="tab-close" v-if="!item.hidden" @click.stop="removeTab(item)"><i class="fa fa-close"></i></div>
                    </li>
                </transition-group>
            </ul>
        </div>
    ...
    methods: {
        ...
        removeTabOther(data) {
            ...
            this.$store.dispatch('DelTabOther', data).then(([next, data]) => {
                for (var k in data) {
                    let item = data[k]
                    if (item.attributes.url == next.attributes.url || item.attributes.url === homeTabData.attributes.url) continue
